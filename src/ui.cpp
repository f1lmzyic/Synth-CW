    ...
    if (lock)
    {
        ...
        else
        {
            ...
            case PAGE_ARP:
                ...
                {
                    if (direction != 0)
                        sysState.params.arpEnabled = !sysState.params.arpEnabled;
                }
                else if (knobIndex == 1)
                {
                    int16_t val = sysState.params.arpMode + direction;
                    if (val < 0)
                        val = 3;
                    if (val > 3)
                        val = 0;
                    sysState.params.arpMode = val;
                }
                else if (knobIndex == 2)
                {
                    int16_t val = sysState.params.arpRate + (direction * 4);
                    if (val < 0)
                        val = 0;
                    if (val > 127)
                        val = 127;
                    sysState.params.arpRate = val;
                }
                else if (knobIndex == 3)
                {
                    int16_t val = sysState.params.arpOctaves + direction;
                    if (val < 0)
                        val = 0;
                    if (val > 2)
                        val = 2;
                    sysState.params.arpOctaves = val;
                }
                break;
            ...
            case PAGE_CHORD:
                ...
                {
                    if (direction != 0)
                        sysState.params.chordEnabled = !sysState.params.chordEnabled;
                }
                else if (knobIndex == 1)
                {
                    int16_t val = sysState.params.chordType + direction;
                    if (val < 0)
                        val = 5;
                    if (val > 5)
                        val = 0;
                    sysState.params.chordType = val;
                }
                else if (knobIndex == 2)
                {
                    int16_t val = sysState.params.chordInversion + direction;
                    if (val < 0)
                        val = 0;
                    if (val > 2)
                        val = 2;
                    sysState.params.chordInversion = val;
                }
                else if (knobIndex == 3)
                {
                    int16_t val = sysState.params.chordSpread + direction;
                    if (val < 0)
                        val = 0;
                    if (val > 2)
                        val = 2;
                    sysState.params.chordSpread = val;
                }
                break;
            ...
            case PAGE_AUTO:
                if (knobIndex == 0)
                {
                    if (direction != 0)
                    {
                        bool turningOn = !sysState.params.autoRecEnabled;
                        sysState.params.autoRecEnabled = turningOn;
                ...
                else if (knobIndex == 1)
                {
                    if (direction != 0)
                    {
                        bool turningOn = !sysState.params.autoPlayEnabled;
                        sysState.params.autoPlayEnabled = turningOn;
                ...
                else if (knobIndex == 2)
                {
                    if (direction != 0)
                    {
                        sysState.params.autoLength = (sysState.params.autoLength == 16) ? 8 : 16;
                        sysState.autoStepIndex = 0;
                        sysState.autoLastStepMs = millis();
                    }
                }
...
void displayUpdateTask(void *pvParameters)
{
    ...
    static const char *pageNames[] = {
        "OSC", "OSC2", "FLT", "MODEL", "ENV", "MOD", "MENV", "S&H",
        "FX", "CHO", "PATCH", "ARP", "CHRD", "AUTO"};

    while (1)
    {
#ifndef TEST_DISPLAY_WCET
        ...
        if (localState.menuMode)
        {
            ...
            case PAGE_ARP:
            {
                ...
                u8g2->setCursor(8, 16);
                sprintf(buf, "On: %s", localState.params.arpEnabled ? "Yes" : "No");
                u8g2->print(buf);

                u8g2->setCursor(8, 24);
                sprintf(buf, "Mode: %s", arpModeName(localState.params.arpMode));
                u8g2->print(buf);

                u8g2->setCursor(72, 16);
                sprintf(buf, "Rate: %d", localState.params.arpRate);
                u8g2->print(buf);

                u8g2->setCursor(72, 24);
                sprintf(buf, "Oct: %d", localState.params.arpOctaves);
                u8g2->print(buf);
                break;
            }
            ...
            case PAGE_CHORD:
            {
                ...
                u8g2->setCursor(8, 16);
                sprintf(buf, "On: %s", localState.params.chordEnabled ? "Yes" : "No");
                u8g2->print(buf);

                u8g2->setCursor(8, 24);
                sprintf(buf, "Type: %s", chordTypeName(localState.params.chordType));
                u8g2->print(buf);

                u8g2->setCursor(72, 16);
                sprintf(buf, "Inv: %d", localState.params.chordInversion);
                u8g2->print(buf);

                u8g2->setCursor(72, 24);
                sprintf(buf, "Spr: %d", localState.params.chordSpread);
                u8g2->print(buf);
                break;
            }
            ...
            case PAGE_AUTO:
            {
                ...
                u8g2->setCursor(8, 16);
                sprintf(buf, "Rec: %s", localState.params.autoRecEnabled ? "On" : "Off");
                u8g2->print(buf);

                u8g2->setCursor(8, 24);
                sprintf(buf, "Play: %s", localState.params.autoPlayEnabled ? "On" : "Off");
                u8g2->print(buf);

                u8g2->setCursor(72, 16);
                sprintf(buf, "Len: %d", localState.params.autoLength);
                u8g2->print(buf);

                u8g2->setCursor(72, 24);
                sprintf(buf, "Step:%02d", localState.autoStepIndex);
                u8g2->print(buf);
        ...
        else
        {
            ...
            if (localState.viewMode == 0)
            {
                ...
                // Latest CAN message (Lab Part 2 check)
                u8g2->setFont(u8g2_font_5x7_tr);
                u8g2->setCursor(72, 31);
