...
int32_t processDelay(EffectsState* state, int32_t input, uint8_t delayTime, uint8_t feedback, uint8_t mix) {
    if (delayTime == 0) return input;
    
    uint32_t delayLen = ((uint32_t)delayTime * DELAY_BUFFER_SIZE) / 128;
    if (delayLen < 100) delayLen = 100;
    
    uint32_t readPos = (state->delayWritePos + DELAY_BUFFER_SIZE - delayLen) % DELAY_BUFFER_SIZE;
...
int32_t processChorus(EffectsState* state, int32_t input, uint8_t rate, uint8_t depth, uint8_t mix) {
    if (depth == 0 || mix == 0) {
    ...
    uint32_t readPos = (state->chorusWritePos + CHORUS_BUFFER_SIZE - delayAmt) % CHORUS_BUFFER_SIZE;
