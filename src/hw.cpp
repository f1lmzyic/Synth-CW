...
void pitchBendTask(void *pvParameters) {
  ...
  // Worst case: pitch bend enabled with joystick at extreme position
  bool pbEnabled = true;

  // Mutex acquisition (worst case includes contention)
  {
    MutexGuard lock(sysState.mutex, pdMS_TO_TICKS(5));
    if (lock) {
  ...
  // Simulate extreme joystick position for worst-case computation
  int16_t joyY = 0;  // Maximum deflection
  ...
  if (pbEnabled) {
    ...
    // Force the update path by resetting lastUpdateMs
    lastUpdateMs = 0;
