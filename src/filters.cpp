...
int32_t processSVF(FilterState &state, int32_t input, uint8_t cutoff,
  ...
  // SVF core: integrate BP to get LP, differentiate LP to get BP, subtract for
  // HP
...
int32_t processMoogFilter(FilterState &state, int32_t input, uint8_t cutoff,
                          uint8_t resonance, uint8_t filterType) {
  ...
  if (filteredInput > 255)
    filteredInput = 255;
  if (filteredInput < -256)
    filteredInput = -256;
  ...
  switch (filterType) {
  case 0:
    return state.moog_s[3]; // Lowpass (4-pole)
  case 1:
    return input - state.moog_s[3]; // Highpass approximation
  case 2:
    return state.moog_s[2] - state.moog_s[3]; // Bandpass approximation
  case 3:
    return input - state.moog_s[2]; // Notch approximation
  default:
    return state.moog_s[3];
  }
}

int32_t processMS20Filter(FilterState &state, int32_t input, uint8_t cutoff,
                          uint8_t resonance, uint8_t filterType) {
  ...
  if (filteredInput > 180)
    filteredInput = 180;
  if (filteredInput < -200)
    filteredInput = -200;
  ...
  switch (filterType) {
  case 0:
    return state.ms20_s[1]; // Lowpass
  case 1:
    return filteredInput - state.ms20_s[0]; // Highpass
  case 2:
    return state.ms20_s[0] - state.ms20_s[1]; // Bandpass
  case 3:
    return state.ms20_s[1] + (filteredInput - state.ms20_s[0]); // Notch
  default:
    return state.ms20_s[1];
  }
}
