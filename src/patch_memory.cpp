...
bool patchIsValid(uint8_t slot) {
  ...
  // Check magic number
  if (metadata->magic != PATCH_MAGIC)
    return false;

  // Check version
  if (metadata->version != PATCH_VERSION)
    return false;
...
bool patchSave(uint8_t slot, const char *name) {
  ...
  if (HAL_FLASHEx_Erase(&eraseInit, &pageError) == HAL_OK) {
    ...
    for (uint32_t i = 0; i < dataSize; i += 8) {
      ...
      if (HAL_FLASH_Program(FLASH_TYPEPROGRAM_DOUBLEWORD, address + i, word) !=
          HAL_OK) {
        success = false;
        break;
      }
...
uint8_t patchGetCurrentSlot() { return currentPatchSlot; }

// Set current loaded patch slot
void patchSetCurrentSlot(uint8_t slot) { currentPatchSlot = slot; }
