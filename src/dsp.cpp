...
void dspUpdateParams() {
    ...
    int8_t newOctaveOffset;
    uint16_t pressedCopy[MAX_PRESSED_KEYS];
...

    {
        ...
        newParams = sysState.params;
        newOctaveOffset = sysState.octaveOffset;
        pressedCount = sysState.pressedKeyCount;
        for (uint8_t i = 0; i < pressedCount; i++) {
            pressedCopy[i] = sysState.pressedKeys[i];
        }
    }
    ...
    // Voice update outside the long critical section
...
void sampleISR() {
    ...
    // 14-17. Master output
    // 14-17. Master output (Lab Part 2 style log taper)
    uint8_t vol = (smoothVol < 0) ? 0 : ((smoothVol > 8) ? 8 : smoothVol);
